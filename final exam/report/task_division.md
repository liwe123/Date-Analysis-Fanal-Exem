...
- `src/collect_corpus.py`：怎么从 Wikipedia 自动扒 28 个词条（重试、限速、中英文回退）
- `src/preprocess.py` 里的 `clean_text()`：怎么去掉 HTML 标签、乱码字符、多余空行

### 汇报词（照着念）
> 我负责第一步，就是把原始文件读进来、洗干净。
>
> 数据源有两类：一是课程资料，`data/raw/` 下面大概 50 个 md 文件，包括课程介绍、FAQ、案例、通知这些；二是我们自动从 Wikipedia 扒的 28 个大数据相关词条，放在 `data/raw/external/`。
>
...
**Q：GPT 提取元数据会不会很慢很贵？**
A：不贵。50 篇文档各调一次 gpt-4o-mini，加起来也就几毛钱。而且这是建库时离线跑一次，不影响用户使用速度。
...
>
> 我用 OpenAI 的 text-embedding-3-small 模型把文本转成 1536 维的向量，存进 ChromaDB。ChromaDB 是个开源的本地向量数据库，不需要装 Docker，数据直接存硬盘上 `vector_store/` 文件夹。
>
...
>
> 我还做了一个查询解析器：用户说「去年老师有没有说过期末怎么考」，我用另一个 GPT 调用把这句话翻译成 `search_query: "期末考试 形式", filters: {year: "2025", category: "notice"}`。解析失败就直接用原问题搜，不会卡住。
...
**Q：embedding 模型为什么不用 ada-002？**
A：text-embedding-3-small 是新模型，效果差不多但便宜 5 倍，而且支持缩维度。

**Q：查询解析也用 GPT，会不会让速度变慢？**
A：多调一次 gpt-4o-mini，大概多花 1-2 秒。但这对用户透明，而且有个好处——用户怎么说话都行，不用学特定格式。失败了也有降级策略。
