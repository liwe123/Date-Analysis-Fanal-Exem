...
def ap(tf, txt, sz=14, c=TX, b=False, a=PP_ALIGN.LEFT, be=0, af=4):
    ...
    p.font.name = "Microsoft YaHei"
    p.alignment = a
    p.space_before = Pt(be)
    p.space_after = Pt(af)
    return p


def hdr(sl, tit, sub=""):
    re(sl, 0, 0, W, Inches(1.0), D)
    re(sl, 0, Inches(0.95), W, Inches(0.05), A)
    tx(sl, Inches(0.6), Inches(0.12), Inches(10), Inches(0.5), tit, 26, WH, True)
    if sub:
        tx(sl, Inches(0.6), Inches(0.55), Inches(10), Inches(0.35), sub, 14, LG)
...
tx(
    ...
    Inches(10),
    Inches(0.4),
    "ChromaDB  |  DeepSeek V4  |  Qwen3-Embedding  |  Streamlit",
    15,
    LG,
    False,
    PP_ALIGN.CENTER,
...
for i, (nu, ti, co) in enumerate(toc):
    col = i // 5
    off = i % 5
    x = Inches(1.2) + Inches(6.0) * col
    y = Inches(1.4) + Inches(1.05) * off
...
for i, (lb, co) in enumerate(layers):
    x = sx + i * (bw + gap)
    sh = re(sl, x, Inches(2.3), bw, bh, co)
...
tx(
    ...
    Inches(12),
    Inches(0.35),
    "输入：data/raw/ -> 176 篇文档（45 篇课程资料 + 83 篇 Wiki + 30 篇 SO + 18 篇 CSDN）",
    14,
    GR,
)
tx(
...
ap(
    tf,
    "数据流：读取 -> 清洗分块 -> LLM 元数据 -> Qwen3 嵌入 -> ChromaDB 存储 -> 解析 -> 混合检索 -> DeepSeek 生成",
    12,
    GR,
)
...
card(
    ...
    Inches(5.8),
    Inches(5.5),
    "语料采集 collect_corpus.py",
    [
        "* 多源语料库高敏捷采集与转换",
        "* Wikipedia 词条 83 篇 (Wiki REST API)",
        "* Stack Overflow 高票问答 30 篇 (译为全中文)",
        "* CSDN 技术博客 18 篇",
        "* 多线程/速率限制 (429) 指数退避重试",
        "* 统一自动转换为纯净 Markdown 存储",
    ],
    T,
)
...
for i, (t, d) in enumerate(
    ...
    re(sl, Inches(0.8), y, Inches(11.5), Inches(0.45), CA)
...
for i, (t, d) in enumerate(
    [
        ("第一层：段落边界", "按空行切分，保持段落语义完整"),
        ("第二层：句子边界", "按句末标点切分，保留完整句子"),
        ("第三层：贪心合并", "合并到 chunk_size=700 字符，重叠 120 字符"),
        ("第四层：滑窗切割", "超长文本以 step=chunk_size-overlap 切割"),
    ]
):
    y = Inches(4.7) + Inches(0.5) * i
    re(sl, Inches(0.8), y, Inches(0.04), Inches(0.4), T)
    tx(sl, Inches(1.1), y, Inches(3.0), Inches(0.4), t, 13, T, True)
    tx(sl, Inches(4.1), y, Inches(8.0), Inches(0.4), d, 12, TX)
...
card(
    ...
    "LLM 元数据提取",
    [
        "* 发送前 1200 字符给 DeepSeek V4",
        "* 返回 JSON：作者/年份/分类/语言/摘要",
        "* Front-Matter 优先覆盖 LLM 结果",
        "* 文件名后缀猜测兜底",
        "* 176 篇文档元数据并发提取",
        "* 32 线程并发提取，仅需 ~20 秒",
    ],
...
tx(
    ...
    Inches(11.5),
    Inches(0.3),
    "合并策略：Front-Matter 人工标注 > LLM 提取 > 文件名猜测 -> 最终写入 ChromaDB",
    15,
    A,
    True,
)
...
tx(
    ...
    Inches(11.5),
    Inches(0.3),
    "降级保障：Where 过滤失败 -> 自动移除过滤条件 -> 纯语义搜索回退 -> 用户永不见空白",
    15,
    O,
    True,
)
...
for i, f in enumerate(
    [
        "* 参数量 0.6B，输出 1024 维向量，支持 32K 上下文",
        "* 中文 MTEB 66.33 同级最优",
        "* 自动检测 CUDA 启用 GPU 加速",
        "* 编码 450 多个文本块仅需 10-20 秒（GPU 加速）",
        "* 通过环境变量可一键切换任意 HuggingFace 模型",
    ]
):
    tx(
        sl,
        Inches(1.0),
        Inches(2.2) + Inches(0.35) * i,
        Inches(11),
        Inches(0.3),
        f,
        13,
...
for i, (x, w) in enumerate(zip(cx, cw)):
    re(sl, x, Inches(4.8), w, Inches(0.45), D)
...
tr = [
    ["本地（当前）", "Qwen3-0.6B", "1024", "0 元", "GPU 10-20 秒"],
    ["远程 API", "text-embed-3-small", "1536", "约 0.01 元", "API 延迟"],
]
for ri, row in enumerate(tr):
    y = Inches(5.3) + Inches(0.45) * ri
    bg2 = CA if ri % 2 == 0 else WH
...
tx(
    ...
    Inches(11),
    Inches(0.4),
    "LLM 自动将用户问题拆解为搜索词加过滤条件，用户无需学习搜索语法。",
    14,
    GR,
)
ex = [
    (
        "用户提问：去年期末怎么考？",
        '解析结果：{"search_query": "期末考试 形式", "filters": {"year": 2025, "category": "notice"}}',
    ),
    (
        "用户提问：2024 年通知讲了啥？",
        '解析结果：{"search_query": "通知", "filters": {"year": 2024, "category": "notice"}}',
    ),
    ("用户提问：什么是向量数据库？", '解析结果：{"search_query": "向量数据库", "filters": null}'),
    (
        "用户提问：张三写的案例分析？",
        '解析结果：{"search_query": "案例分析", "filters": {"author": "张三"}}',
    ),
]
for i, (q, a) in enumerate(ex):
    y = Inches(2.0) + Inches(1.0) * i
    re(sl, Inches(0.8), y, Inches(11.7), Inches(0.85), CA)
...
tx(
    ...
    Inches(11),
    Inches(0.5),
    "降级保护：JSON 解析异常或 API 调用失败时，自动使用原问题全文搜索，系统永不卡死。",
    13,
    O,
    True,
)
...
bg(sl, WH)
hdr(sl, "四、答案生成与防幻觉机制")
for i, (nu, ti, de, co) in enumerate(
    [
        ("1", "System Prompt 约束", "LLM 被要求仅依据\n参考资料回答，找不到\n就说不知道。", G),
        ("2", "上下文隔离", "只将检索结果喂给\nLLM，不给模型自我\n发挥的空间。", G),
        ("3", "引用强制检查", "后端自动检查答案中\n是否含 [Source:]，\n缺失则自动补全。", G),
    ]
):
    x = Inches(0.6) + Inches(4.2) * i
    re(sl, x, Inches(1.5), Inches(3.8), Inches(3.2), CA)
...
tx(
    ...
    Inches(11.5),
    Inches(0.3),
    "工程规范：类型注解全覆盖 | 模块间单向依赖 | 显式 init_env | OpenAI 单例 | 分层错误处理",
    15,
    P,
    True,
)
...
for i, (la, va, cl) in enumerate(
    ...
    y = Inches(2.2) + Inches(0.55) * i
    tx(sl, Inches(7.2), y, Inches(3.0), Inches(0.35), la, 12, GR)
    tx(
        sl,
        Inches(7.2),
        y,
...
hl = [
    ...
    (
        "安全删除机制",
        "delete_collection 需显式\n确认 confirm=True，防止\n误删数据。",
        P,
    ),
]
for i, (ti, de, co) in enumerate(hl):
    x = Inches(0.6) + Inches(6.2) * (i % 2)
    y = Inches(1.4) + Inches(1.85) * (i // 2)
    re(sl, x, y, Inches(5.9), Inches(1.6), CA)
    re(sl, x, y, Inches(0.06), Inches(1.6), co)
    tx(
        sl,
        x + Inches(0.25),
        y + Inches(0.15),
...
td = [
    ["向量数据库", "ChromaDB", "Milvus/Qdrant", "零部署 pip install 即用"],
...
bg(sl, WH)
hdr(sl, "八、局限与改进方向")
for i, (li, im, so) in enumerate(
    [
        ("PDF 仅提取文字", "含图表的文档信息丢失", "增加表格和图片提取模块"),
...
qa = [
    ("问：", "Embedding API 限流怎么办？", "答：", "本地 GPU 嵌入，完全离线不存在限流问题"),
