...
def build() -> None:
    ...
    pdf.body(
        ...
        "实质性编造仅在 Q44（Python for循环）中发生。经深层次的底层数据追踪，我们发现早期退化"
        "主要来自 ChromaDB 对多键 where 隐式 AND 条件的校验限制：当查询同时携带年份、类别等多"
        "个元数据约束时，部分条件组合需要显式写成 $and。当前 `embed_store.py` 已将多键 where "
        "自动转换为显式 $and，并在失败时保留安全回退；后续优化重点转向元数据口语化解析和低相似度"
        "引用抑制。"
    )
    ...
    pdf.body(
        "第二个是早期版本中获得 4 分的亚健康案例：当用户输入“2025年的通知有哪些？”时，意图解析器成功"
        "提取出了 `{year: 2025, category: 'notice'}`。但在向量距离比对中，由于知识库中"
        " 2025 年的特定通知样本量较为稀疏，加之早期 where 多键条件尚未统一转换为显式 $and，"
        "系统自动执行安全回退，平滑降级为全局语义相似度搜索。大模型最终基于语义关联召回了 2024 "
        "年底发出的下一年度预备通知，虽然信息主体高度相关且保证了系统未卡死，但在年限精确度上"
        "存在轻微偏移；当前代码已补齐多键 where 到显式 $and 的兼容转换。第三个则是典型的 1 分失效幻觉案例：当恶意输入属于超纲技术问题的“Python "
    ...
    pdf.body(
        ...
        "早期复合条件 SQL 过滤退化。在处理复杂的多级 filters 查询时，多键 where 若以隐式 AND "
        "形式传入 ChromaDB，可能触发 `Expected where to have exactly one operator` 校验错误，"
        "导致检索强制降级为纯语义检索。我们的修复方案是：在 `embed_store.py` 检索层增加兼容"
        "映射，将大模型输出的多级 JSON 字典在传入数据库前扁平化为标准单级 dict，并主动重写为"
        "严格的 `{$and: [...]}` 嵌套字典格式，从而消除这类隐式 AND 兼容风险。"
    )
    pdf.body(
