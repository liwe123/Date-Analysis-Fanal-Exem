...
核心交付：从杂乱文本文件到可搜索向量索引、再到带来源追溯的智能问答，全链路自动化。项目采用"行走的骨架"策略——先跑通端到端最小流水线，再逐步增强。最终系统具备 CLI 命令行问答与 Streamlit Web 界面两种交互方式，支持多轮对话、混合检索（语义向量 + 元数据过滤）、来源追溯与查询意图解析。
...
└──────────┬───────────────────────────────────────────────────────────┘
     ...
     │            │  collect_corpus.py (Wikipedia REST API → md)
     └─────┬──────┘
           │
     ┌─────▼──────┐
     │  2. 处理    │  preprocess.py
...
在 Streamlit 界面中，每次回答附可展开的"检索来源"面板，显示每条命中文档的距离、路径和片段预览。
...
2. 展示侧边栏：文档块数、来源数、来源文件列表
3. 连续提问 2 个问题（含追问），展示对话历史
4. 开启调试模式：展示查询解析的中间结果（搜索词 + 过滤条件）
5. 调节 Top-K 滑块（3→10），查看检索结果数量变化
6. 展开"📎 检索来源"，展示距离分数和文本片段
7. 调节 max_distance 阈值，展示精度/召回权衡
```
