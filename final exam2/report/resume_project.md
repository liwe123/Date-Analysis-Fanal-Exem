...
- 覆盖 91 个 pytest 测试用例（单元 / 集成 / 端到端），支持本地 GPU 嵌入与远程 API 双模式；提供 Streamlit Web 界面与 CLI 双入口
...
- 实现 Prompt Engineering 防幻觉策略（系统约束 + 上下文限定 + 来源强制），确保生成答案可追溯至原始文档，覆盖 91 个测试用例保证系统稳定性
...
- 编写了 91 个 pytest 测试用例，覆盖边界条件与降级路径
