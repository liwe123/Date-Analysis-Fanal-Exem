...
def build_defense_qa_pdf():
    ...
    pdf.bullet_list([
        "tests/ 文件夹：里面包含 test_ingest.py、test_preprocess.py、test_embed_store.py、test_qa.py、test_rendering.py 等 7 个专业自动化测试模块，涵盖全套 91 个单测及集成测试用例。",
        "运行命令：在终端运行 python -m pytest tests/ -v，即可看到 91 个用例全部 PASS 的高水平表现。"
    ])
    
    pdf.paragraph("2. 必须记住的大白话专业名词生词本：")
    pdf.bullet_list([
        "Pytest：主流的 Python 自动化测试框架。我编写的 91 个用例可以在约 3 秒内自动校验全部模块输入输出，极大提升了项目的软件工程规范度。",
    ...
    pdf.draw_qa_box(
        ...
        "在持续集成（CI）或离线测试环境下，网络超时或缺少私钥是阻碍自动化测试的最大痛点。为了解决这一问题，我全面采用了 Python 标准库中的 unittest.mock.patch 装饰器对外部 OpenAI/DeepSeek 接口、ChromaDB 连接以及 Hugging Face Embedding 下载环境实施了高保真度无物理依赖的 Mock 模拟。我为测试设计了固定的黄金返回数据和预期的异常抛出路径，使得全套 91 个单测与集成测试用例可以在无网环境下一键在约 3 秒内跑完，并实现了 100% 成功通过，彻底证明了代码的高可靠性。"
    )
