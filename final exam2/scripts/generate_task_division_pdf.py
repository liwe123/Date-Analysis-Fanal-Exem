...
class TaskDivisionPdf(FPDF):
    ...
    def draw_timeline(self):
        ...
        steps = [
            ...
            ("第六阶段", "刘洋", "测试用例开发与文档归纳", "编写 82 个全覆盖测试用例，归纳 HNSW 容灾备份"),
...
def build_task_division_pdf():
    ...
    rows = [
        [
            ...
            "3. 技术攻坚：定位并攻克 Mock 绑定路径穿透引起的单测报错隐患，将 Pytest 测试套件全面升级并顺利通过 82 个全量测试用例；统筹项目最终交付与远程代码库同步。"
        ],
        [
            "张杰 (组员)\n[数据工程师]",
            "数据工程师",
        ...
        [
            "刘洋 (组员)\n[测试与文档工程师]",
            "测试与文档工程师",
            "1. 单元与集成测试：使用 pytest 编写覆盖全链路的单测用例（共 19 类 82 个），包含 YAML 嵌套解析、API 降级和 System 信号穿透等极端边缘用例的断言验证。\n"
    ...
    col_desc_text = (
        ...
        "并最终在 82 个单元与集成测试中实现了 100% 成功通过 (100% Passed)，系统质量与文档质量均达到优秀的工业级交付标准。"
    )
    ...
    pdf.bullet_list([
        "基于 Mock 的 Pytest 全覆盖测试设计 (tests/)：构建了涵盖所有核心组件的自动化测试包，包含 19 个独立测试类，共 82 个详尽的单元测试与端到端集成测试用例，覆盖率为团队开发之最。全面采用了 unittest.mock.patch 装饰器对外部 OpenAI 接口和 Hugging Face Embedding 下载环境实施无物理依赖的完美模拟，使得开发环境和测试管道可在无网状态下瞬时运行全套测试。",
