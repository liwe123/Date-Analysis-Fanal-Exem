...
def build_task_division_pdf():
    ...
    rows = [
        ...
        [
            "刘洋 (组员)\n[测试与文档工程师]",
            "测试与文档工程师",
            "1. 单元与集成测试：使用 pytest 编写覆盖全链路的 91 个测试用例，包含 YAML 嵌套解析、API 降级和 System 信号穿透等极端边缘用例的断言验证。\n"
    ...
    pdf.bullet_list([
        "基于 Mock 的 Pytest 全覆盖测试设计 (tests/)：构建了涵盖所有核心组件的自动化测试包，共 91 个详尽的单元测试与端到端集成测试用例，覆盖率为团队开发之最。全面采用了 unittest.mock.patch 装饰器对外部 OpenAI 接口和 Hugging Face Embedding 下载环境实施无物理依赖的完美模拟，使得开发环境和测试管道可在无网状态下瞬时运行全套测试。",
