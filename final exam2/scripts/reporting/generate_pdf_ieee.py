...
def build() -> None:
    ...
    pdf.body(
        ...
        "正确拒答），评估脚本标记幻觉 4/9，且 91 项 pytest 测试用例全部通过，"
    ...
    pdf.btable(
        ...
        [
            ...
            ["幻觉率(超纲)", "4/9 (44.4%)", "超纲查询中 4 次被评估标记为幻觉，主口径以脚本结果为准"],
    ...
    pdf.body(
        ...
        "幻觉率为 44.4%（4/9），越界拒答准确率为 55.56%（5/9）。这 4 个失败案例既包含真正"
        "偏离课程知识库的问题，也包含模型已经拒答但仍携带无关来源引用的保守判定场景。当前"
        "`embed_store.py` 已经将多键 where 条件转换为显式 `$and`，因此复合过滤不再作为待修复"
        "缺陷表述；后续优化重点应放在越界召回抑制、拒答时来源展示策略以及 out-of-scope 判定"
        "阈值校准上。"
    )
    ...
    pdf.body(
        ...
        "安全防线，在评估脚本口径下取得 55.56% 的越界拒答准确率，并把剩余 4/9 的失败案例明确纳入"
        "后续召回抑制和拒答展示策略优化清单，使得系统具备持续演进的工业投产素养。"
    )
